...
1. **`docs/internal/known-limitations.md` 정독** — v0.5.0 시점 살아있는 잠재 과제 단일 진실원. v1.0 이후 발견된 항목도 여기 누적
2. **현재 worklog 최신 파일 확인** — `worklog/` 의 최근 결정 (지금 9개 유지: 009 / 011 / 014-020). 이전 세션의 결정 이어받기
3. **historical 컨텍스트가 필요하면 `archive/`** — `archive/HANDOVER_2026_05_14.md` (lets_portal 분리 시점) + `archive/worklog/` (처리된 release 기록 12 + html). git history 보존, 검색 가능
...
### 6-b. 금지 의존성 (2026-05-24 영구 결정)

**다음 의존성은 어떤 패키지에도 추가 금지.** 의사결정 반복 금지 — 추가 검토 시
`worklog/009_no_libreoffice_decision.md` 의 § 3 진짜 근거를 먼저 읽을 것.

| 금지 의존성 | 이유 (요약) |
|---|---|
| **LibreOffice** (`soffice` CLI, `libreoffice-*` apt 패키지) | system 의존성 200MB+, CI 시간 +5분/PR, RAM +500MB, 안정성 리스크 |
| **unoconv** | 내부적으로 soffice 호출 — LibreOffice 의 alias |
| **pdf2image** (`poppler-utils` 의존) | system binary, ~30MB + poppler 의존 |
| **tesseract** (`tesseract-ocr` apt 패키지) | system binary, 언어팩별 부담 |
| **wkhtmltopdf** | system binary, Qt 의존 |

**원칙:** 새 의존성 후보는 "**Python 패키지 1종으로 처리 가능한가**" 가 첫 질문.
가능 → 검토 진행. 불가능 → 위 worklog § 3 의 6가지 부담(Docker / CI / RAM /
변환시간 / 안정성 / 보안) 을 모두 평가한 뒤만 예외 채택.

**영향:** 7포맷 → **5포맷 (PDF/DOCX/HWP/HWPX/PPTX)**. DOC/PPT 는 사용자가 직접
신버전으로 변환 후 업로드 (`korean-doc-parser` 책임 밖).
...
자세한 정책 적용 예시는 `archive/worklog/004_kordoc_review.md` 참고.
...
## 출시 진행 상황 (2026-05-25 기준)

처리된 release 기록은 모두 `archive/worklog/` (000-008 / 010 / 012 / 013).
의사결정 + 살아있는 reference 는 `worklog/` 에 유지.

- ✅ Phase 0 (의사결정) — `archive/worklog/001_phase0_benchmark_decision.md`
- ✅ v0.1.0 / v0.1.1 — PDF / DOCX / HWPX (`archive/worklog/002`, `005`)
- ✅ v0.2.0 / v0.2.1 — HWP + PDF 비트맵 (`archive/worklog/007`, `008`)
- ✅ v0.3.0 — PPTX markitdown 위임 (`archive/worklog/010`)
- ✅ v0.4.0~v0.4.5 — Vision CLI + 평가 + stats + 5포맷 이미지 추출 완성
  (`worklog/014` ~ `017`)
- ✅ v0.5.0 — Pipeline 알고리즘 모듈 + default Haiku + 코드 부채 정리
  (`worklog/019` 의 b 안 + `worklog/020` release)
- ⏳ **v1.0.0** — 사내 stable lock. archive 정리 (이 commit) + 호출자 문서
  (README + docs/ko/recipes) 박은 후 진입

**다음 세션 시작 시 가장 먼저 읽을 reference:**
1. `docs/internal/known-limitations.md` (살아있는 잠재 과제 통합)
2. `worklog/020_v0.5.0_release.md` (현 시점 마일스톤)
3. `worklog/019_v0.5_b_decision.md` (Pipeline = 알고리즘만 결정 명문화)
...
> **사내 private 라이브러리.** 단일 파일 포터빌리티 + 패키지 경계 + SemVer + 회귀 테스트 의무. 영문 docs/외부 SLA 등 공개 OSS 의무는 삭제. 핵심 원칙(이유/제약/실패 만 기록)은 lets_portal 과 동일. v0.5.0 이후 컨텍스트 진입점은 `docs/internal/known-limitations.md` + 최신 worklog. historical 은 `archive/`.
