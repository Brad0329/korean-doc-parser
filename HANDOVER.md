...
> **이 문서 위치:** 본 파일은 `lets_portal/work_log2/` 에 생성됐습니다. 새 프로젝트 `doc_parser/` 디렉토리를 만든 후 이 파일을 **`doc_parser/HANDOVER.md`** 로 이동시키세요.
>
> **작성일:** 2026-05-14 (lets_portal Phase D 진행 중, file_parser 1차 작업 직후)
> **다음 세션에서:** 이 문서 § 0 → § 1~10 순서로 정독한 뒤 작업 시작

---

## 0. 이 문서 사용법

- 새 프로젝트 `doc_parser/` 의 **유일한 시작점**입니다. 처음 세션 열어서 이 문서만 정독하면 컨텍스트 100% 복원됩니다.
- 본문 안 **❓ 표시 항목**은 작업 시작 전 사용자 결정 필요. 답하지 않고 코드 시작 금지.
- **참조 산출물**: § 5 의 "가져갈 파일 목록" — lets_portal repo 에서 복사할 파일들.
- **첫 작업 시 절차**: § 9 의 체크리스트 그대로 진행.
