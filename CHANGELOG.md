...
(다음 마일스톤은 v0.5 — Pipeline 패키지 + PostgreSQL + 비동기 큐. v0.5.0 에서
default 모델 **Sonnet → Haiku** 전환 결정 박힘 — worklog/014 § 4-2 의 진짜 근거:
검수 큐 정렬이 Haiku 의 calibrate 된 confidence 에서만 의미를 가짐.

자산화 우선 경로 (worklog/014 § 5-5) 의 다음 후보: PPTX 이미지 추출
(worklog/014 § 5-1) 또는 HWP 이미지 추출 (worklog/007 § 2-3).)
