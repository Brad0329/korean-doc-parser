...
(v1.0.0 stable lock 도달. 이후 변경은 의식적인 trade-off 로 — 호환 깨짐은
deprecation cycle 후 v2.0.0 에서. worklog/021 § 5-3 의 v2.0.0 시나리오 참고.)

---

## [1.0.0] — 2026-05-25

**Major — 사내 stable lock.** v0.5.0 의 코드 자산 + Phase 7 archive 정리 +
Phase 8 호출자 문서가 v1.0 의 산출물. **새 코드 0건**, public API 호환성
약속 시작점.

자세한 내용은 `worklog/021_v1.0.0_release.md` 참고.

### Stable surface (호환성 약속)

- `korean_doc_parser.{extract, ExtractedImage, ParseResult, ParsedTable,
  ParseMetadata, BboxUnit, BaseParser, ParserRegistry, ParseError, ...}`
- `korean_doc_parser.pipeline.{compute_doc_id, weighted_confidence,
  detect_caption_regex, detect_caption_proximity, TextBlock, CaptionDetection,
  DEFAULT_WEIGHTS}`
- `korean_doc_parser.vision.{VisionClient, VisionResult, VisionCache,
  CachedLabel}` (extras `[vision]`)
- CLI `kdp-label` (단일 이미지 / `--from-document` / `--stats`)

### 호출자 진입 가능

- **bidwatch** → [`docs/ko/recipes/bidwatch.md`](docs/ko/recipes/bidwatch.md)
- **vanasso.kr** → [`docs/ko/recipes/vanasso.md`](docs/ko/recipes/vanasso.md)
- **사내 RAG** → README + recipes 활용

### Documented (Phase 7-8)

- `README.md` — v0.2.1 stale → v0.5.0 / 5포맷 / pipeline / Vision CLI 갱신
- `docs/ko/recipes/{bidwatch,vanasso}.md` — 호출자 패턴
- `docs/internal/known-limitations.md` — 메인테이너 reference 5 카테고리
- `archive/HANDOVER_2026_05_14.md` + `archive/worklog/` — historical 보존
- `CLAUDE.md` — 새 세션 진입점 갱신, 출시 진행 상황 v0.5.0 까지

### Quality gates (실측)

- pytest: **232 통과** (v0.5.0 동일)
- 전체 라인 커버리지: **91%+**
- ruff + mypy strict + pre-commit: 모두 통과
- 벤치마크 회귀: **-21.3% (개선)** vs v0.2.0_baseline, worklog/018

### v1.0.0 이후 정책

- 사내 stable: 호환 깨짐 변경은 v2.0.0 까지 미루기 (또는 deprecation cycle)
- 운영 발견 케이스 → 회귀 테스트 영구 추가
- 잠재 patch 후보 (BMP / chart / page_no 등) 는 호출자 가치 발생 시 진입
  (docs/internal/known-limitations.md 참고)
