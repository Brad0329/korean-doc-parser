...
**다음 마일스톤은 v0.4 — `kdp-label` Vision CLI 만.** 작업량 ~2-3일.

v0.4 종합 의사결정 (20+ 항목) 은 `worklog/011_v0.4_decision_matrix.md` 에 박힘.
2단계 평가 패턴 (worklog/010 § 2-2) 재사용 — Vision CLI 평가 후 v0.5 Pipeline
진입 결정.

### Planned (v0.4)

- **`kdp-label` CLI** (`packages/core/src/korean_doc_parser/cli/label.py`):
  - 이미지 1장 또는 `--from-document` 로 문서에서 추출된 이미지 일괄 라벨링
  - Claude Sonnet (A.1) + 4필드 2-pass (B.1, confidence < 0.7 만 reasoning)
  - 비트맵 100x100 px 이상만 (B.2), 한국어 caption (B.3), OCR 없음 (B.4)
  - caption post-process 마스킹 (C.2 — 주민번호/전화/이메일 regex)
  - SQLite 임시 캐시 (C.4 sha256 + model, v0.5 에서 PostgreSQL 로 마이그)
  - cost_krw / cache_hit 등 stdout JSON 출력 (A.4)
- **새 의존성:** `anthropic>=0.40` (extras `[vision]`, MIT)
- **합격 기준** (worklog/011 § 9): N≥5 이미지 평가에서 caption 한국어 ≥30% +
  image_type 정확 ≥4/5 + confidence 분포 60-80% auto_approved + 비용 ≤$0.01/img
