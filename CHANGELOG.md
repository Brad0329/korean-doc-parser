...
**다음 마일스톤은 v0.3 — PPTX (markitdown 위임).** 작업량 ~1-2일.

**영구 정책 변경 (2026-05-24):** LibreOffice 도입 금지로 v0.3 scope 가 크게
축소되었습니다. 자세한 근거는 `worklog/009_no_libreoffice_decision.md` 참고.

### Changed (policy)

- **7포맷 → 5포맷** — DOC / PPT 는 LibreOffice 의존성 회피 결정으로 **영구
  미지원**. 사용자가 신버전(.docx/.pptx) 으로 직접 변환 후 업로드.
- **금지 의존성 정책** 도입 (`CLAUDE.md` § 6-b) — LibreOffice / unoconv /
  pdf2image / tesseract / wkhtmltopdf 등 system binary 의존성 영구 금지

### Planned (v0.3)

- **PPTX 파서** — markitdown 위임 (Phase 0 벤치마크 결과 markitdown 23K vs 자체 170)
- **새 의존성:** `markitdown` (Python 패키지, 가벼움, system 의존성 없음)
- **OS 매트릭스 변경 없음**

### Deferred to v0.4 or later

- Pipeline 패키지 — DocumentIngestionPipeline + Claude Vision 라벨링
- 검수 큐 API (FastAPI) + 검수 UI
- HWP 이미지 추출 (XHTML `bindata/` 경로 → `ExtractedImage` 매핑)
