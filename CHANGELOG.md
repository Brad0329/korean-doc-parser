...
- **DOCX 파서** (`korean_doc_parser.parsers.docx.DocxParser`, python-docx 기반):
  - 텍스트(`Heading 1` / `제목 1` 등 → ``#`` 마크다운 헤더 변환),
    ...
    이미지(ZIP 안 `word/media/*` 직접 추출 → tempfile + sha256)
  - 메타데이터: `title`, `author`, `created_at`, `modified_at` (core_properties)
  - DOCX 는 페이지 개념 없음 → `page_no=None`
  - 합성 픽스처 3건 (simple / with_table / with_image) +
    테스트 12건, 라인 커버리지 97%
- **HWPX 파서** (`korean_doc_parser.parsers.hwpx.HwpxParser`, XML 직접 파싱):
  - namespace-agnostic 워크 (`local-name` 매칭) — 한/글 실 schema + 합성 모두 호환
  - 텍스트: `<*:p>` 단위 paragraphs, `<*:t>` 모두 결합 → markdown
  - 표: `<*:tbl>` → `ParsedTable[]`, 표 paragraph 의 셀 텍스트는 markdown 중복 방지
  - 메타데이터 `title`: `Contents/content.hpf` 의 `<opf:title>`
  - 이미지: `BinData/*` + `Contents/Resources/*` → tempfile + sha256
  - 합성 픽스처 2건 (simple / with_table) + 테스트 13건, 라인 커버리지 83%
    (이미지 픽스처 미포함 — v0.2 에서 확장)
